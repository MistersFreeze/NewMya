...
-- animatedgui.lua was loaded synchronously via HttpGet here; on slow/blocked networks that blocks the entire Fluent init (inject appears "frozen" before any window).
local Animation = { Apply = function() end }
...
local aa = {
    function()
        ...
        SaveManager.Parser = {
            ...
            Colorpicker={ Save=function(idx,o) return{type="Colorpicker",idx=idx,value=o.Value:ToHex(),transparency=o.Transparency,rainbow=(o.Rainbow==true)} end, Load=function(idx,d) if SaveManager.Options[idx] then SaveManager.Options[idx]:SetValueRGB(Color3.fromHex(d.value),d.transparency,d.rainbow==true) end end },
        ...
        InterfaceManager.Settings = { Theme="Blood Red", Acrylic=true, Transparency=true, Animated=true, MenuKeybind="RightShift", Font="GothamSSm" }
                        ...
                        Buttons = {
                            {
                                Title = "Yes",
                                Callback = function()
                                    local ranUnload = false
                                    local knownHooks = {"unload_new_mya", "unload_mya", "unload_mya_universal"}
                                    for _, hookName in ipairs(knownHooks) do
                                        local fn = rawget(_G, hookName)
                                    ...
                                    for gk, gv in pairs(_G) do
                                        if typeof(gk) == "string" and typeof(gv) == "function" then
                                            local lk = string.lower(gk)
                                            if gk ~= "unload_new_mya"
                                                and string.sub(lk, 1, 7) == "unload_"
                                                and (string.find(lk, "mya", 1, true) or string.find(lk, "aftermath", 1, true)) then
                                                ranUnload = true
                                                pcall(gv)
                                            end
                                        end
                                    end
                                    if not ranUnload then
                                        p:Destroy()
                                    end
                                end
                            },
    ...
    [21] = function()
        ...
        function u.New(v, w, x)
            ...
            local z = {
                ...
                Rainbow = x.Rainbow == true,
                RainbowSpeed = tonumber(x.RainbowSpeed) or 0.15,
                RainbowConn = nil,
                Type = "Colorpicker",
            ...
            function z.StopRainbow(A)
                if z.RainbowConn then
                    z.RainbowConn:Disconnect()
                    z.RainbowConn = nil
                end
            end
            function z.StartRainbow(A)
                z:StopRainbow()
                z.Rainbow = true
                z.RainbowConn = l:Connect(function(dt)
                    z.Hue = (z.Hue + dt * z.RainbowSpeed) % 1
                    z:Display()
                end)
            end
            ...
            local aa, ab =
                ...
                function()
                    ...
                    C.Root.Size = UDim2.fromOffset(430, 370)
                    local D, E, F, G, H, I =
                        z.Hue,
                        z.Sat,
                        z.Vib,
                    ...
                    local ac
                    local rainbowOn = z.Rainbow == true
                    ...
                    local function updateRainbowButtonLabel()
                        if rainbowBtn then
                            rainbowBtn.Text = "  Rainbow: " .. (rainbowOn and "On" or "Off")
                        end
                    end
                    local function startRainbow()
                        stopRainbow()
                        rainbowConn = l:Connect(function(dt)
                            D = (D + dt * z.RainbowSpeed) % 1
                            ac()
                        end)
                    end
                    ...
                    p.AddSignal(
                        ...
                        function(ad)
                            if
                             ...
                             then
                                disableRainbow()
                                while ad.UserInputState ~= Enum.UserInputState.End do
                    ...
                    rainbowBtn =
                        ...
                        },
                        {
                            ...
                            s("UIStroke", {Transparency = 0.5, ThemeTag = {Color = "DialogButtonBorder"}}),
                        }
                    )
                    p.AddSignal(
            ...
            function z.SetValue(ac, ad, ae, af)
                local ag = Color3.fromHSV(ad[1], ad[2], ad[3])
            ...
            function z.SetValueRGB(ac, ad, ae, af)
                z.Transparency = ae or 0
                z:SetHSVFromRGB(ad)
                z:Display()
                if af == true then
